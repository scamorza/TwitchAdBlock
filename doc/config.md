...
| `BackupPlayerTypes` | `['embed','popout','autoplay']` | Order in which a clean stream is looked for. `embed` and `popout` carry the full ladder; `autoplay` is capped around 360p but is the most often clean. | Reorder only with measurements. Dropping `autoplay` means some breaks have no backup at all and fall to stripping. |
| `FallbackPlayerType` | `'embed'` | Used when no player type is currently serving. | Rarely. |
